    ...
    <div class="Overlay" ref="overlay" v-show="show">
      <div class="registering">
        <div class="registerLoading">
          <van-loading size="36px" text-size="16px" vertical>登录中...</van-loading>
        </div>
      </div>
    </div>
  </div>
...
export default {
  computed: {
    ...mapState(["user", "psd1", "psd2", "checkBool", "show", "homebool"])
  },
  watch: {
    // 监听登录验证是否通过。通过跳转,设置用户登录状态
    homebool: function(newvalue, oldvalue) {
      if (newvalue == true) {
        var logininfo = { user: this.user, islogin: true };
        sessionStorage.setItem("loginstatus", JSON.stringify(logininfo));
  ...
  methods: {
    ...
    showOL() {
      this.$store.commit("loginModule/showOL");
    }
  },
  mounted() {
    let registerH =
  ...
  beforeRouteLeave(to, from, next) {
    this.user.name = this.psd1.item = "";
    this.user.message = this.psd1.message = "";
    next();
  }
};
</script>